    ...
    <Paper withBorder p="lg" radius="md" className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 shadow-xs">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        <div>
          ...
          <Text size="xs" c="dimmed" mt={1}>
            All Base Settlement Rates quoted in <span className="font-semibold text-slate-800 dark:text-slate-200">Myanmar Kyat (MMK)</span>
          </Text>
        </div>
        ...
        <Group gap="xs">
          <Button
            ...
            size="xs"
            color="corporateBlue"
            className="bg-blue-50 dark:bg-blue-900/30 text-[#0F4C81] dark:text-blue-300 hover:bg-blue-100 dark:bg-blue-800/40 font-semibold"
        ...
        {displayedCurrencies.map((rate) => {
          ...
          return (
            <div
              key={rate.currency}
              onClick={() => setIsCurrencyConverterOpen(true)}
              className="p-3.5 rounded-lg border border-slate-100 dark:border-slate-700/50 bg-slate-50 dark:bg-slate-800/50 hover:bg-white dark:hover:bg-slate-900 hover:border-[#0F4C81]/30 hover:shadow-xs transition-all cursor-pointer group"
            >
              ...
              <div className="flex items-baseline justify-between mt-1">
                <span className="text-xl font-black text-[#0F4C81] dark:text-blue-300 font-mono group-hover:text-blue-700 transition-colors">
                  {formatNumber(rate.middleRate)}
                </span>
                <span className="text-[10px] text-slate-400 dark:text-slate-500 font-medium">MMK</span>
              </div>

              <div className="grid grid-cols-2 gap-1.5 mt-2 pt-2 border-t border-slate-200 dark:border-slate-700/60 text-[10px]">
                <div>
                  <span className="text-slate-400 dark:text-slate-500 block text-[9px]">Buy Rate:</span>
                  <span className="font-semibold text-slate-700 dark:text-slate-300 font-mono">{formatNumber(rate.buyRate)}</span>
                </div>
                <div className="text-right">
                  <span className="text-slate-400 dark:text-slate-500 block text-[9px]">Sell Rate:</span>
                  <span className="font-semibold text-slate-700 dark:text-slate-300 font-mono">{formatNumber(rate.sellRate)}</span>
                </div>
              </div>
            </div>
          );
      ...
      <div className="mt-3 flex justify-center">
        <button
          ...
          className="text-xs text-[#0F4C81] dark:text-blue-300 hover:text-blue-800 font-semibold flex items-center gap-1 cursor-pointer py-1 px-3 rounded hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors"
        >
